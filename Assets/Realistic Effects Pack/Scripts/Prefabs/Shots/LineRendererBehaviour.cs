...
public class LineRendererBehaviour : MonoBehaviour
{
    ...
    void InitializeDefault()
    {
        ...
        else
        {
            ...
            if (Is2D)
            {
                Vector2 direction2D = targetDirection; // This works
                Debug.Log("Laser direction: " + direction2D.ToString());
                
                RaycastHit2D hit2D = Physics2D.Raycast(tRoot.position, targetDirection, effectSettings.MoveDistance + 1, effectSettings.LayerMask);
                Debug.Log("Laser collision point: " + hit2D.point.ToString());
                line.SetPosition(1, hit2D.point - effectSettings.ColliderRadius * direction2D);

                var particlesOffsetPos = hit2D.point - direction2D * ParticlesHeightOffset;
                if (StartGlow != null) StartGlow.transform.position = tRoot.position;
                if (HitGlow != null) HitGlow.transform.position = particlesOffsetPos;
                if (GoLight != null) GoLight.transform.position = hit2D.point - direction2D * LightHeightOffset;
                if (Particles != null) Particles.transform.position = particlesOffsetPos;
                if (Explosion != null)
                {
                    Explosion.transform.position = particlesOffsetPos;
                    Explosion.transform.LookAt(particlesOffsetPos + hit2D.normal);
                }
            } // end if Is2D
            else if (Physics.Raycast(tRoot.position, targetDirection, out hit, effectSettings.MoveDistance + 1, effectSettings.LayerMask))
            {
