...
public class LanguageEditor : EditorWindow
{
	...
	static string settingsAssetPath = "Assets/Resources/Languages/LocalizationSettings.asset";
        ...
        get
        {
            if (m_settings == null)
            {
                if (File.Exists(settingsAssetPath))
                    m_settings = (LocalizationSettings)AssetDatabase.LoadAssetAtPath(settingsAssetPath, typeof(LocalizationSettings));
                else
                {
		...
		{		
			...
			foreach(HtmlAgilityPack.HtmlNode tdNode in trNode.SelectNodes(".//td"))	
			{
				...
				if(firstRow && row == 0) {
                    ...
                    if (tdNode.InnerText == "")
                        continue;
                    Debug.Log("First row and 0: " + tdNode.InnerText);
					...
					if(i == 0) {
                        ...
                        Debug.Log("i = 0: " + tdNode.InnerText);
						continue;
					}
					if(key == "")
						continue; //Skip entries with empty keys (the other values can be used as labels)
